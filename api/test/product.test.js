...
test("Read all and each product's information", async () => {
  await supertest.get(`/product/${productId}`).send().expect(200);

  await supertest.get("/products").send().expect(200);
});
...
test("Delete a specific product without permission", async () => {
  await supertest.delete(`/product/${productId}`).send().expect(401);
});
